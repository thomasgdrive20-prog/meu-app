...
const Tag = ({ children, color, small }) => (
  <span style={{
    background: (color || T.treino) + '18', color: color || T.treino,
    fontSize: small ? 9 : 10, padding: small ? '3px 10px' : '4px 13px',
    borderRadius: 100, letterSpacing: .8, fontWeight: 700,
    textTransform: 'uppercase', border: `1px solid ${(color || T.treino)}28`, display: 'inline-block',
  }}>{children}</span>
)
...
const Btn = ({ children, onClick, color, small, full, ghost, disabled, style = {} }) => (
  <button onClick={onClick} disabled={disabled} className={ghost ? 'btn-ghost' : 'btn-neon'} style={{
    background: ghost ? 'transparent' : (color || T.treino),
    border: ghost ? `1px solid ${(color || T.treino)}50` : 'none',
    color: ghost ? (color || T.treino) : '#0A0A0B',
    borderRadius: 14, padding: small ? '9px 18px' : '14px 24px',
    fontSize: small ? 12 : 13, fontWeight: 800,
    cursor: disabled ? 'not-allowed' : 'pointer', fontFamily: 'inherit',
    width: full ? '100%' : 'auto', opacity: disabled ? .4 : 1, letterSpacing: .5, ...style
  }}>{children}</button>
)

const Inp = ({ placeholder, value, onChange, type = 'text', style = {} }) => (
  <input type={type} placeholder={placeholder} value={value || ''} onChange={e => onChange(e.target.value)}
    style={{ background: T.faint, border: `1px solid ${T.border2}`, borderRadius: 12, padding: '12px 14px', fontSize: 13, color: T.text, outline: 'none', fontFamily: 'inherit', ...style }}
    onFocus={e => e.target.style.borderColor = T.treino + '80'}
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', padding: '20px 0' }}>
      ...
      <div style={{ display: 'flex', gap: 10 }}>
        ...
        <Btn small color={T.treino} onClick={onDone} style={{ display: 'flex', alignItems: 'center', gap: 6 }}><SkipForward size={14} /> Pular</Btn>
      </div>
    </div>
  )
}
...
function WorkoutMode({ session, exercises, onClose, onSave }) {
  ...
  const nextSet = () => {
    if (kg && !isCardio) { const entry = { exercise: ex.name, kg, reps, date: new Date().toLocaleDateString('pt-BR'), day: session.id }; onSave(entry); setSaved(s => [...s, entry]) }
    if (setIdx2 + 1 < totalSets) { setSetIdx(s => s + 1); if (ex.rest > 0) setPhase('rest'); else { setKg(''); setReps('') } }
    else { setSetIdx(0); setKg(''); setReps(''); if (idx + 1 < exercises.length) { setIdx(i => i + 1); setPhase('exercise') } else setPhase('done') }
  }
  ...
  if (phase === 'done') return (
    <div style={{ position: 'fixed', inset: 0, background: T.bg, zIndex: 100, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', padding: 32 }}>
      ...
      {saved.slice(-5).map((s, i) => <div key={i} style={{ fontSize: 12, color: T.text, marginBottom: 6, opacity: .6 }}>{s.exercise} — {s.kg}kg × {s.reps}</div>)}
  ...
  return (
    <div style={{ position: 'fixed', inset: 0, background: T.bg, zIndex: 100, overflowY: 'auto' }}>
      ...
      <div style={{ padding: '52px 24px 120px' }}>
        ...
        <div style={{ fontSize: 13, color: T.muted, marginBottom: 28, fontWeight: 600, display: 'flex', alignItems: 'center', gap: 6 }}><Timer size={13} /> Série {setIdx2 + 1} de {totalSets}</div>
        <div style={{ display: 'flex', gap: 8, marginBottom: 28 }}>
          {Array.from({ length: totalSets }, (_, i) => (
            <div key={i} style={{ width: 40, height: 40, borderRadius: '50%', border: `2px solid ${i < setIdx2 ? session.color : i === setIdx2 ? session.color : T.border2}`, background: i < setIdx2 ? session.color : i === setIdx2 ? session.color + '20' : 'transparent', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 11, fontWeight: 800, color: i < setIdx2 ? T.bg : i === setIdx2 ? session.color : T.muted, boxShadow: i === setIdx2 ? `0 0 10px ${session.color}60` : 'none' }}>{i + 1}</div>
          ))}
        </div>
        ...
        <div style={{ display: 'flex', gap: 12, marginBottom: 24 }}>
          {[{ l: 'Reps alvo', v: ex.reps, Icon: Target }, { l: 'Pausa', v: ex.rest > 0 ? `${ex.rest}s` : '—', Icon: Clock }].map(s => (
            <div key={s.l} style={{ flex: 1, background: T.surface, border: `1px solid ${T.border}`, borderRadius: 18, padding: '18px', textAlign: 'center' }}>
              <s.Icon size={16} color={session.color} style={{ marginBottom: 8 }} />
              <div style={{ fontSize: 26, fontWeight: 900, color: session.color, letterSpacing: -1 }}>{s.v}</div>
        ...
        <button onClick={() => { setIdx(i => Math.min(i + 1, exercises.length - 1)); setSetIdx(0); setPhase('exercise') }} style={{ background: 'none', border: 'none', color: T.muted, fontSize: 12, cursor: 'pointer', width: '100%', marginTop: 16, textAlign: 'center', fontFamily: 'inherit', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 6 }}>
          <SkipForward size={13} /> Pular exercício
        </button>
      </div>
    </div>
  )
...
export default function App() {
  ...
  useEffect(() => {
    ...
    Promise.all([dbSelect('workout_logs'), dbSelect('body_metrics'), dbSelect('meal_logs'), dbSelect('exames'), dbSelect('aplicacoes'), dbSelect('dieta_logs'), dbSelect('photos'), dbSelect('supl_logs')])
      .then(([wl, bm, ml, ex, ap, dl, ph, sl]) => {
        ...
        setMealLog(ml); setLabLog(ex); setAplicacoes(ap); setDietaLogs(dl); setPhotos(ph); setSuplLogs(sl)
  ...
  const TABS = [
    ...
    { id: 'fotos',  Icon: Image,    label: 'Fotos',  color: T.gold    },
  ]
  ...
  return (
    <div style={{ minHeight: '100vh', background: T.bg, fontFamily: "'Barlow','Lato',sans-serif", color: T.text, maxWidth: 480, margin: '0 auto', paddingBottom: 90 }}>
      ...
      {workoutMode && <WorkoutMode session={workoutMode.session} exercises={workoutMode.exercises} onClose={() => setWorkoutMode(null)} onSave={addWorkout} />}
      ...
      <div className="glass" style={{ padding: '48px 24px 16px', display: 'flex', justifyContent: 'space-between', alignItems: 'center', position: 'sticky', top: 0, zIndex: 50 }}>
        <div>
          <div style={{ fontSize: 28, fontWeight: 900, letterSpacing: -1.5, lineHeight: 1 }}>{profile.name}</div>
          <div style={{ fontSize: 11, color: T.muted, marginTop: 4, fontWeight: 600, display: 'flex', alignItems: 'center', gap: 6 }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
          ...
          <div style={{ width: 44, height: 44, borderRadius: '50%', background: `linear-gradient(135deg,${T.treino},#8BC34A)`, display: 'flex', alignItems: 'center', justifyContent: 'center', color: T.bg, fontWeight: 900, fontSize: 16, boxShadow: `0 0 16px ${T.treino}40` }}>T</div>
        </div>
      </div>
      ...
      {/* BOTTOM NAV */}
      <div className="glass-nav" style={{ position: 'fixed', bottom: 0, left: '50%', transform: 'translateX(-50%)', width: '100%', maxWidth: 480, padding: '10px 8px 22px' }}>
        <div style={{ display: 'flex', justifyContent: 'space-around' }}>
          {TABS.map(t => {
            const active = tab === t.id
            return (
              <button key={t.id} onClick={() => setTab(t.id)} style={{ background: active ? t.color + '15' : 'none', border: 'none', cursor: 'pointer', display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 5, padding: '8px 12px', borderRadius: 16, transition: 'all .2s', position: 'relative' }}>
                {active && <div style={{ position: 'absolute', top: -1, left: '50%', transform: 'translateX(-50%)', width: 24, height: 2, background: t.color, borderRadius: 2, boxShadow: `0 0 8px ${t.color}` }} />}
                <t.Icon size={20} color={active ? t.color : T.muted} strokeWidth={active ? 2 : 1.5} style={{ transition: 'all .2s' }} />
                <span style={{ fontSize: 8, color: active ? t.color : T.muted, textTransform: 'uppercase', letterSpacing: 1, fontWeight: 800 }}>{t.label}</span>
              </button>
            )
          })}
        </div>
  ...
  return (
    <div>
      ...
      <div onClick={todaySession?.id !== 'off' ? onStartWorkout : undefined} className={todaySession?.id !== 'off' ? 'card-tap' : ''}
        style={{ background: `linear-gradient(135deg,${T.card} 0%,${(todaySession?.color || T.treino) + '12'} 100%)`, border: `1px solid ${(todaySession?.color || T.treino) + '30'}`, borderRadius: 24, padding: '24px', marginBottom: 12, cursor: todaySession?.id !== 'off' ? 'pointer' : 'default', transition: 'all .2s' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 14, marginBottom: todaySession?.id !== 'off' ? 16 : 0 }}>
          <div style={{ fontSize: 48 }}>{todaySession?.icon}</div>
          <div>
      ...
      <Card>
        ...
        {[{ l: 'Dieta', v: dietaPct, c: T.nutri, Icon: Utensils }, { l: 'Treino', v: treinoPct, c: T.treino, Icon: Dumbbell }, { l: 'Protocolo', v: protoPct, c: T.horm, Icon: Syringe }, { l: 'Suplementos', v: suplPct, c: T.gold, Icon: Pill }].map(s => (
          <div key={s.l} style={{ marginBottom: 14 }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: 11, marginBottom: 7, fontWeight: 700, alignItems: 'center' }}>
      ...
      {isInjDay && !alreadyInjected && (
        <Card color={T.horm}>
          ...
          <Btn full color={T.horm} style={{ color: T.bg }} onClick={() => { addAplicacao({ compound: 'Testosterona Enantato', dose: '0.7', unit: 'ml', obs: '', date: today }); addAplicacao({ compound: 'Anastrozol', dose: '0.5', unit: 'mg', obs: '', date: today }) }}>Registrar Aplicação</Btn>
        </Card>
      )}
      {isInjDay && alreadyInjected && (
...
function TreinoTab({ workout, addItem, editItem, removeItem, onStartMode }) {
  ...
  if (sel) {
    ...
    return (
      <div>
        ...
        <div style={{ fontSize: 30, fontWeight: 900, color: session.color, marginBottom: 4, letterSpacing: -1 }}>{session.icon} {session.label}</div>
        <div style={{ fontSize: 12, color: T.muted, marginBottom: 16, fontWeight: 600 }}>{session.tag}</div>
        <Btn full color={session.color} style={{ marginBottom: 16, color: T.bg, display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8 }} onClick={() => onStartMode(session, EXERCISES[sel] || [])}><Play size={16} fill={T.bg} /> MODO TREINO — TELA CHEIA</Btn>
  ...
  return (
    <div>
      <div style={{ fontSize: 30, fontWeight: 900, letterSpacing: -1.5, marginBottom: 4, display: 'flex', alignItems: 'center', gap: 12 }}><Dumbbell size={28} color={T.treino} strokeWidth={1.5} /> Treinos</div>
      ...
      <div style={{ display: 'flex', gap: 8, marginBottom: 16, overflowX: 'auto', paddingBottom: 4 }}>
        {SPLIT.map((s, i) => { const isToday = i === todayIdx; return (
          <div key={s.id} onClick={() => setSel(s.id)} style={{ flexShrink: 0, width: 60, background: isToday ? s.color : T.card, border: `1px solid ${isToday ? s.color : T.border}`, borderRadius: 16, padding: '12px 6px', textAlign: 'center', cursor: 'pointer', transition: 'all .2s', boxShadow: isToday ? `0 0 14px ${s.color}50` : 'none' }}>
      ...
      {SPLIT.map(s => (
        <Card key={s.id} color={s.color} onClick={() => setSel(s.id)}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            <div><div style={{ fontSize: 20, fontWeight: 900, color: s.color, letterSpacing: -.5 }}>{s.icon} {s.label}</div><div style={{ fontSize: 12, color: T.muted, marginTop: 3, fontWeight: 600 }}>{s.tag}</div>{EXERCISES[s.id] && <div style={{ fontSize: 11, color: T.muted, marginTop: 6 }}>{EXERCISES[s.id].length} exercícios</div>}</div>
...
function NutriTab({ nutrition, mealLog, dietaLogs, addMeal, removeMeal, addDietaLog }) {
  ...
  return (
    <div>
      <div style={{ fontSize: 30, fontWeight: 900, letterSpacing: -1.5, marginBottom: 4, display: 'flex', alignItems: 'center', gap: 12 }}><Utensils size={26} color={T.nutri} strokeWidth={1.5} /> Dieta</div>
      ...
      <Card color={T.nutri}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10 }}>
          {[{ l: 'Calorias', cur: todayKcal, meta: nutrition.calories, u: 'kcal', c: T.nutri, Icon: Flame }, { l: 'Proteína', cur: todayProt, meta: nutrition.protein, u: 'g', c: T.treino, Icon: Zap }].map(m => (
            <div key={m.l} style={{ background: T.faint, borderRadius: 16, padding: '16px' }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginBottom: 8 }}><m.Icon size={14} color={m.c} strokeWidth={1.5} /><div style={{ fontSize: 10, color: T.muted, fontWeight: 700, textTransform: 'uppercase', letterSpacing: 1 }}>{m.l}</div></div>
              ...
              <div style={{ background: T.border, borderRadius: 4, height: 5, overflow: 'hidden' }}><div style={{ width: `${Math.min((m.cur / m.meta) * 100, 100)}%`, height: '100%', background: `linear-gradient(90deg,${m.c}CC,${m.c})`, borderRadius: 4, transition: 'width 1s ease' }} /></div>
            </div>
          ))}
        </div>
      </Card>
        ...
        {DIET.map((meal, i) => { const status = getMealStatus(meal.id); const urg = getUrg(meal.time); return (
          <div key={meal.id} style={{ padding: '14px 0', borderBottom: i < DIET.length - 1 ? `1px solid ${T.border}` : 'none' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 6 }}>
              ...
              {status ? <Tag color={status.ate ? T.ok : T.alert} small>{status.ate ? '✓ Comi' : '✗ Não'}</Tag> : (
                <div style={{ display: 'flex', gap: 6 }}>
                  <button onClick={() => toggleMeal(meal, true)} style={{ background: T.ok + '18', color: T.ok, border: `1px solid ${T.ok}35`, borderRadius: 10, padding: '6px 14px', fontSize: 11, fontWeight: 800, cursor: 'pointer', fontFamily: 'inherit' }}>✓</button>
                  <button onClick={() => toggleMeal(meal, false)} style={{ background: T.alert + '18', color: T.alert, border: `1px solid ${T.alert}35`, borderRadius: 10, padding: '6px 14px', fontSize: 11, fontWeight: 800, cursor: 'pointer', fontFamily: 'inherit' }}>✗</button>
                </div>
              )}
            </div>
      ...
      {mealLog.filter(m => m.date === todayDate).length > 0 && (
        <Card><Lbl>Livres de Hoje</Lbl>{mealLog.filter(m => m.date === todayDate).map((m, i, arr) => (
          <div key={m.id} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '9px 0', borderBottom: i < arr.length - 1 ? `1px solid ${T.border}` : 'none' }}>
            ...
            <button onClick={() => removeMeal(m.id, m.desc)} style={{ background: 'none', border: 'none', color: T.alert, cursor: 'pointer' }}><X size={16} /></button>
          </div>
        ))}</Card>
      )}
    </div>
...
function HormTab({ labLog, addLab, removeLab, aplicacoes, addAplicacao, removeAplicacao, compounds, saveCompounds }) {
  ...
  return (
    <div>
      <div style={{ fontSize: 30, fontWeight: 900, letterSpacing: -1.5, marginBottom: 4, display: 'flex', alignItems: 'center', gap: 12 }}><Heart size={26} color={T.horm} strokeWidth={1.5} /> Saúde</div>
      <div style={{ fontSize: 12, color: T.muted, marginBottom: 18, fontWeight: 600 }}>Protocolo · Aplicações · Exames</div>
      {alerts.length > 0 && (
        <Card color={T.alert}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 12 }}><AlertTriangle size={18} color={T.alert} /><Lbl color={T.alert}>Alertas</Lbl></div>
      ...
      <Card color={T.horm}>
        ...
        {compounds.map((c, idx) => (
          <div key={c.id} style={{ padding: '14px 0', borderBottom: idx < compounds.length - 1 ? `1px solid ${T.border}` : 'none' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
              <div><div style={{ fontSize: 14, fontWeight: 800 }}>{c.name}</div><div style={{ fontSize: 11, color: T.muted, marginTop: 3, fontWeight: 600, display: 'flex', alignItems: 'center', gap: 6 }}><Calendar size={11} /> {c.schedule} · {c.weekly}/sem</div></div>
              {editDose?.idx === idx ? (
      ...
      <div style={{ marginBottom: 10 }}><Btn full color={T.horm} style={{ color: T.bg, display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8 }} onClick={() => setShowApp(!showApp)}><Syringe size={16} /> {showApp ? 'Cancelar' : 'Registrar Aplicação'}</Btn></div>
      {showApp && (
        <Card color={T.horm}>
      ...
      {aplicacoes.length > 0 && (<Card><Lbl>Histórico de Aplicações</Lbl>{[...aplicacoes].sort((a, b) => new Date(b.created_at || b.date) - new Date(a.created_at || a.date)).slice(0, 12).map((a, i) => (<div key={a.id} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '9px 0', borderBottom: `1px solid ${T.border}` }}><div><div style={{ fontSize: 13, fontWeight: 800 }}>{a.compound}</div><div style={{ fontSize: 10, color: T.muted, marginTop: 2 }}>{a.date}{a.obs ? ` · ${a.obs}` : ''}</div></div><div style={{ display: 'flex', alignItems: 'center', gap: 10 }}><Tag color={T.horm}>{a.dose}{a.unit}</Tag><button onClick={() => removeAplicacao(a.id, `${a.compound} ${a.date}`)} style={{ background: 'none', border: 'none', color: T.alert, cursor: 'pointer' }}><X size={15} /></button></div></div>))}</Card>)}
      <div style={{ marginBottom: 10, marginTop: 6 }}><Btn full color={T.nutri} style={{ color: T.bg, display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8 }} onClick={() => setShowLab(!showLab)}><FlaskConical size={16} /> {showLab ? 'Cancelar' : 'Registrar Exame'}</Btn></div>
      {showLab && (
        <Card color={T.nutri}>
          <Lbl>Novo Exame</Lbl>
      ...
      {labLog.length > 0 && (<Card><Lbl>Histórico de Exames</Lbl>{[...labLog].sort((a, b) => new Date(b.created_at || b.date) - new Date(a.created_at || a.date)).slice(0, 20).map((l, i) => { const st = getStatus(l.marker, l.value); const trend = getTrend(l.marker); const ref = LAB_REFS[l.marker]; const TrendIcon = trend === 'up' ? TrendingUp : trend === 'down' ? TrendingDown : Minus; const trendColor = trend === 'up' ? T.alert : trend === 'down' ? T.ok : T.muted; return (<div key={l.id} style={{ display: 'flex', alignItems: 'flex-start', padding: '9px 0', borderBottom: `1px solid ${T.border}` }}><StatusDot status={st} /><div style={{ flex: 1 }}><div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}><div><div style={{ fontSize: 13, fontWeight: 800 }}>{l.marker}</div><div style={{ fontSize: 10, color: T.muted, marginTop: 2 }}>{l.date || l.created_at?.slice(0, 10)}{ref ? ` · ${ref.min}–${ref.max}${ref.unit}` : ''}</div></div><div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>{trend && <TrendIcon size={14} color={trendColor} />}<span style={{ fontSize: 14, fontWeight: 800, color: st === 'ok' ? T.ok : st === 'warn' ? T.warn : T.alert }}>{l.value}{l.unit ? ` ${l.unit}` : ''}</span><button onClick={() => removeLab(l.id, l.marker)} style={{ background: 'none', border: 'none', color: T.alert, cursor: 'pointer' }}><X size={14} /></button></div></div></div></div>) })}</Card>)}
      <Card color={T.treino}>
...
function BodyTab({ metrics, profile, addMetric, editMetric, removeMetric }) {
  ...
  return (
    <div>
      <div style={{ fontSize: 30, fontWeight: 900, letterSpacing: -1.5, marginBottom: 4, display: 'flex', alignItems: 'center', gap: 12 }}><Activity size={26} color={T.metrica} strokeWidth={1.5} /> Corpo</div>
      ...
      <Card color={T.metrica}>
        ...
        <div style={{ display: 'flex', gap: 12, alignItems: 'center' }}>
          <div style={{ flex: 1, textAlign: 'center', background: T.faint, borderRadius: 16, padding: '18px 10px' }}><div style={{ fontSize: 38, fontWeight: 900, color: T.alert, letterSpacing: -2 }}>{last?.bf || '—'}%</div><div style={{ fontSize: 9, color: T.muted, textTransform: 'uppercase', letterSpacing: 2, marginTop: 6, fontWeight: 700 }}>Atual</div></div>
          <ArrowRight size={24} color={T.muted} />
          <div style={{ flex: 1, textAlign: 'center', background: T.faint, borderRadius: 16, padding: '18px 10px' }}><div style={{ fontSize: 38, fontWeight: 900, color: T.ok, letterSpacing: -2 }}>{profile.bfMeta || 12}%</div><div style={{ fontSize: 9, color: T.muted, textTransform: 'uppercase', letterSpacing: 2, marginTop: 6, fontWeight: 700 }}>Meta</div></div>
        </div>
      </Card>
      {last && (<div style={{ display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: 8, marginBottom: 12 }}>{[{ l: 'Peso', v: `${last.weight}kg`, c: T.treino, Icon: Scale }, { l: 'BF%', v: `${last.bf}%`, c: T.metrica, Icon: Target }, { l: 'Cintura', v: last.waist ? `${last.waist}cm` : '—', c: T.nutri, Icon: Activity }].map(s => (<div key={s.l} style={{ background: T.card, border: `1px solid ${T.border}`, borderRadius: 16, padding: '16px 10px', textAlign: 'center' }}><s.Icon size={14} color={s.c} strokeWidth={1.5} style={{ marginBottom: 8 }} /><div style={{ fontSize: 20, fontWeight: 900, color: s.c, letterSpacing: -1 }}>{s.v}</div><div style={{ fontSize: 9, color: T.muted, textTransform: 'uppercase', letterSpacing: 1.5, marginTop: 6, fontWeight: 700 }}>{s.l}</div></div>))}</div>)}
      <Card color={T.metrica}>
      ...
      {chartData.length > 1 && (<Card><Lbl>Evolução</Lbl><ResponsiveContainer width="100%" height={130}><AreaChart data={chartData}><defs><linearGradient id="gk2" x1="0" y1="0" x2="0" y2="1"><stop offset="5%" stopColor={T.treino} stopOpacity={.2} /><stop offset="95%" stopColor={T.treino} stopOpacity={0} /></linearGradient><linearGradient id="gm2" x1="0" y1="0" x2="0" y2="1"><stop offset="5%" stopColor={T.metrica} stopOpacity={.2} /><stop offset="95%" stopColor={T.metrica} stopOpacity={0} /></linearGradient></defs><XAxis dataKey="n" tick={{ fill: T.muted, fontSize: 9 }} axisLine={false} tickLine={false} /><YAxis hide /><Tooltip contentStyle={{ background: T.card, border: `1px solid ${T.border2}`, borderRadius: 12, fontSize: 11, color: T.text }} /><Area type="monotone" dataKey="kg" stroke={T.treino} strokeWidth={2.5} fill="url(#gk2)" dot={{ fill: T.treino, r: 3 }} connectNulls name="Peso" /><Area type="monotone" dataKey="bf" stroke={T.metrica} strokeWidth={2.5} fill="url(#gm2)" dot={{ fill: T.metrica, r: 3 }} connectNulls name="BF%" /></AreaChart></ResponsiveContainer></Card>)}
      {sorted.length > 0 && (<Card><Lbl>Histórico</Lbl>{[...sorted].reverse().map(m => (<div key={m.id}>{editing?.id === m.id ? (<div style={{ padding: '12px 0', borderBottom: `1px solid ${T.border}` }}><div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 6, marginBottom: 10 }}>{[{ l: 'Peso', k: 'weight' }, { l: 'BF%', k: 'bf' }, { l: 'Cintura', k: 'waist' }, { l: 'Peito', k: 'chest' }, { l: 'B.Esq', k: 'armL' }, { l: 'B.Dir', k: 'armR' }].map(f => (<div key={f.k}><div style={{ fontSize: 9, color: T.muted, marginBottom: 4, fontWeight: 700 }}>{f.l}</div><Inp value={editing[f.k] || ''} onChange={v => setEditing({ ...editing, [f.k]: v })} style={{ width: '100%' }} /></div>))}</div><div style={{ display: 'flex', gap: 8 }}><Btn small color={T.ok} style={{ color: T.bg }} onClick={() => { editMetric(editing.id, { weight: editing.weight, bf: editing.bf, waist: editing.waist, armL: editing.armL, armR: editing.armR, chest: editing.chest }); setEditing(null) }}>✓ Salvar</Btn><Btn small ghost color={T.muted} onClick={() => setEditing(null)}>Cancelar</Btn></div></div>) : (<div style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '10px 0', borderBottom: `1px solid ${T.border}` }}><div style={{ flex: 1, display: 'flex', gap: 10, flexWrap: 'wrap', alignItems: 'center' }}><span style={{ fontSize: 10, color: T.muted, minWidth: 60, fontWeight: 600 }}>{m.date}</span><span style={{ fontWeight: 800, color: T.treino, fontSize: 13 }}>{m.weight}kg</span><span style={{ color: T.metrica, fontSize: 13, fontWeight: 800 }}>{m.bf}%</span>{m.waist && <span style={{ fontSize: 11, color: T.muted }}>C:{m.waist}</span>}{m.armL && <span style={{ fontSize: 11, color: T.muted }}>BE:{m.armL}</span>}</div><button onClick={() => setEditing({ ...m })} style={{ background: 'none', border: 'none', color: T.horm, cursor: 'pointer' }}>✏️</button><button onClick={() => removeMetric(m.id, `Medição ${m.date}`)} style={{ background: 'none', border: 'none', color: T.alert, cursor: 'pointer' }}><X size={15} /></button></div>)}</div>))}</Card>)}
    </div>
  )
}
...
function FotosTab({ photos, addPhoto, removePhoto }) {
  const [view, setView] = useState('grid'); const [selectedA, setSelectedA] = useState(null); const [selectedB, setSelectedB] = useState(null); const [tag, setTag] = useState('frente'); const fileRef = useRef(null)
  ...
  const sorted = [...photos].sort((a, b) => new Date(b.isoDate || 0) - new Date(a.isoDate || 0)); const byTag = (t) => sorted.filter(p => p.tag === t)

  if (view === 'compare') return (
    <div>
      <button onClick={() => { setView('grid'); setSelectedA(null); setSelectedB(null) }} style={{ background: 'none', border: 'none', color: T.muted, cursor: 'pointer', fontSize: 13, marginBottom: 20, fontFamily: 'inherit', fontWeight: 700, display: 'flex', alignItems: 'center', gap: 6 }}><ChevronLeft size={16} /> Voltar</button>
      ...
      {selectedA && selectedB ? (
        <div>
          ...
          <div style={{ display: 'flex', gap: 8, marginTop: 12 }}><div style={{ flex: 1, background: T.faint, borderRadius: 12, padding: '12px', textAlign: 'center' }}><div style={{ fontSize: 9, color: T.muted, letterSpacing: 2, fontWeight: 700 }}>ANTES</div><div style={{ fontSize: 13, fontWeight: 800, color: T.text, marginTop: 4 }}>{selectedA.date}</div></div><div style={{ flex: 1, background: T.faint, borderRadius: 12, padding: '12px', textAlign: 'center' }}><div style={{ fontSize: 9, color: T.muted, letterSpacing: 2, fontWeight: 700 }}>DEPOIS</div><div style={{ fontSize: 13, fontWeight: 800, color: T.ok, marginTop: 4 }}>{selectedB.date}</div></div></div>
        ...
        <div>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8 }}>
            {sorted.map(p => { const isA = selectedA?.id === p.id; const isB = selectedB?.id === p.id; return (
              <div key={p.id} onClick={() => { if (!selectedA) setSelectedA(p); else if (!selectedB && p.id !== selectedA.id) setSelectedB(p) }} style={{ position: 'relative', aspectRatio: '3/4', borderRadius: 16, overflow: 'hidden', cursor: 'pointer', border: `2px solid ${isA ? T.warn : isB ? T.ok : T.border}` }}>
  ...
  return (
    <div>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 18 }}>
        <div><div style={{ fontSize: 30, fontWeight: 900, letterSpacing: -1.5, color: T.gold, display: 'flex', alignItems: 'center', gap: 12 }}><Image size={26} color={T.gold} strokeWidth={1.5} /> Fotos</div><div style={{ fontSize: 12, color: T.muted, marginTop: 2, fontWeight: 600 }}>{photos.length} foto{photos.length !== 1 ? 's' : ''}</div></div>
      ...
      <Card color={T.gold}>
        ...
        <div style={{ display: 'flex', gap: 8, marginBottom: 14, flexWrap: 'wrap' }}>
          {TAGS.map(t => <button key={t} onClick={() => setTag(t)} style={{ background: tag === t ? T.gold + '22' : T.faint, border: `1px solid ${tag === t ? T.gold : T.border2}`, color: tag === t ? T.gold : T.muted, borderRadius: 12, padding: '8px 16px', fontSize: 11, fontWeight: 800, cursor: 'pointer', fontFamily: 'inherit', textTransform: 'capitalize', transition: 'all .2s', boxShadow: tag === t ? `0 0 10px ${T.gold}30` : 'none' }}>{t}</button>)}
        </div>
        <input ref={fileRef} type="file" accept="image/*" capture="environment" onChange={handleFile} style={{ display: 'none' }} />
        <div style={{ display: 'flex', gap: 8 }}>
          <Btn full color={T.gold} style={{ color: T.bg, display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8 }} onClick={() => fileRef.current?.click()}><Camera size={16} /> Câmera</Btn>
          <Btn full ghost color={T.gold} style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8 }} onClick={() => { fileRef.current.removeAttribute('capture'); fileRef.current?.click(); setTimeout(() => fileRef.current?.setAttribute('capture', 'environment'), 500) }}><FolderOpen size={16} /> Galeria</Btn>
        </div>
      </Card>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8 }}>
            {byTag(t).map(p => (
              <div key={p.id} style={{ position: 'relative', aspectRatio: '3/4', borderRadius: 18, overflow: 'hidden', border: `1px solid ${T.border}` }}>
                <img src={p.src} style={{ width: '100%', height: '100%', objectFit: 'cover' }} />
                <div style={{ position: 'absolute', inset: 0, background: 'linear-gradient(transparent 60%,rgba(0,0,0,.85))' }} />
                ...
                <button onClick={() => removePhoto(p.id)} style={{ position: 'absolute', top: 10, right: 10, background: 'rgba(0,0,0,.7)', border: `1px solid rgba(255,255,255,.1)`, color: 'white', width: 30, height: 30, borderRadius: '50%', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center' }}><X size={14} /></button>
              </div>
            ))}
          </div>
        </div>
      ))}
      {photos.length === 0 && (<div style={{ textAlign: 'center', padding: '60px 20px', color: T.muted }}><Camera size={56} color={T.border2} strokeWidth={1} style={{ marginBottom: 16 }} /><div style={{ fontSize: 18, fontWeight: 900, color: T.text, marginBottom: 8, letterSpacing: -1 }}>Sem fotos ainda</div><div style={{ fontSize: 13 }}>A primeira foto é o ponto de partida.</div></div>)}
    </div>
  )
}
